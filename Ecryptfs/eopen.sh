...
show_help() {
    show_copywrite "eCryptfs Folder Opener"
    echo -e "\n\tUsage: eopen file [file [..]]\n"
    echo -e "\tOPTION\tDESCRIPTION"
    echo -e "\tfile\tThe file that you want to open with eCryptfs"
    exit 0
}
...
for var in "$@"; do
    mount -t ecryptfs "$var" "$var" ||
    (echo -e "ERROR: Could not open \"$var\" encrypted folder!" && exit 1)
done
